...
class TblManager():
    """
    The user is responsible for selecting the table topic that corresponds to 
    the table file.
    """
    ...
    def __init__(self, mission, target, flt_server_path): 
        ...
        self.cfe_tbl_file = CfeFile(mission, target)
        self.tbl_data_modified = False
        self.tbl_path_filename = None

        self.top_row = ['Parameter', 'Value']
        self.help_text = HelpText()
        self.window_title = WINDOW_TITLE
        
        # Updated on click events
        self.row = None
        self.col = None
    ...
    def create_window(self):
        ...
        window = sg.Window(self.window_title, self.layout, resizable=True, finalize=True)        
        
        return window

    def select_tbl_topic(self):
        tbl_topic = self.select_tbl_topic_popup()
        if tbl_topic != None:
            if tbl_topic == self.cfe_tbl_file.eds_mission.TOPIC_TBL_TITLE_KEY:
    ...
    def select_tbl_topic_popup(self):
        tbl_topic = None
        topic_list = list(self.cfe_tbl_file.topic_dict.keys())
    ...
    def load_file(self, path_filename, tbl_topic):
        self.tbl_path_filename = path_filename
        print(f'self.tbl_path_filename: {self.tbl_path_filename}')
        err_str, tbl_data_array = self.cfe_tbl_file.read(self.tbl_path_filename,tbl_topic)
        if err_str == None:
            if len(tbl_data_array) > 6:  # TODO: Pick number that ensures at least the file headers exist
                # TODO: Remove testing backdoor 
                if self.tbl_path_filename is None:
                    self.tbl_path_filename = 'FILE_MGR_SYS_TBL_TLM'
                self.window_title = f'{WINDOW_TITLE}: {os.path.basename(self.tbl_path_filename)}'
    ...
    def post_save_file_processing(self, err_str, new_path_filename=None):
        if err_str is None:
            if new_path_filename != None:
                self.tbl_path_filename = new_path_filename
                self.window_title = f'{WINDOW_TITLE}: {os.path.basename(self.tbl_path_filename)}'
            self.reset_tbl_data_modified()
        else:
            sg.popup(err_str, title='Save File Error', keep_on_top=True, non_blocking=True, grab_anywhere=True, modal=False)        

    def execute(self, path_filename=None):
        """
        The current value observer must be created after the GUI window is created and the
        first window read is performed. This supports two workflows:
        
        1. Execute() is called with a filename (typically from File Browser
           Filename provided -> Select corresponding topic ID -> Load data from file
           
        2. File->Open is selected from the main menu_layout
           Select topic ID -> Select corresponding file -> Load data from file
        ...
        if path_filename != None:
            tbl_topic = self.select_tbl_topic()
            if tbl_topic != None:
                self.load_file(path_filename, tbl_topic)
            ...
            if event in (sg.WIN_CLOSED, 'Close', 'Exit') or event is None:       
                if self.tbl_data_modified:
                    ...
                    if save_file == 'Yes':
                        ...
                        self.cfe_tbl_file.write_file(path_filename)
                break

            elif isinstance(event, tuple) and event[0] == '-TABLE-' and event[1] == '+CLICKED+':
                if self.cfe_tbl_file.data_loaded:
                ...
                if self.cfe_tbl_file.data_loaded and not self.row == None:
                    ...
                    self.window['-TABLE-'].update(values=self.cfe_tbl_file.update_data_array(self.row,1,values['-DATA-']))
                    print(f'self.cfe_tbl_file.tbl_data_array[self.row][1]: {self.cfe_tbl_file.tbl_data_array[self.row][1]}')
                ...
                if tbl_topic != None:
                    ...
                    path_filename = sg.popup_get_file('', no_window=True, default_path=self.flt_server_path, initial_folder=self.flt_server_path, file_types=(("Table Files", "*.tbl"),), default_extension=TBL_FILE_EXT) # , history=True)
                    if path_filename != None:
                        self.load_file(path_filename, tbl_topic)
            
            elif event == 'Save':
                err_str = self.cfe_tbl_file.write()
                self.post_save_file_processing(err_str)

            elif event == 'Save As...':
                path_filename = sg.popup_get_file('Save File', save_as=True, no_window=True, initial_folder= self.flt_server_path)
                print(f'path_filename: {path_filename}')
                if path_filename != None:
                    err_str = self.cfe_tbl_file.write_file(path_filename)
                    self.post_save_file_processing(err_str, new_path_filename=path_filename)
...
if __name__ == '__main__':
    ...
    if filename is None:
        print('TblManager starting without a filename')
    else:
        print(f'TblManager staring with file {filename}')
        
    tbl_manager = TblManager(MISSION, CFS_TARGET, FLT_SERVER_PATH)
    tbl_manager.execute(filename) 
