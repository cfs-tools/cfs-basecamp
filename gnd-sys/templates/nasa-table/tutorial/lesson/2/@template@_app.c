...
** Local functions
*/
static int32 SendTblDataEvent(void);
static int32 ValidateExampleTbl(void *TblData);
//EX1
...
//EX2,5,1,
    ...
    ** Register Table
    */
    status = CFE_TBL_Register(&@TEMPLATE@_Data.TblHandle, @TEMPLATE@_EX_TBL_CFE_NAME, sizeof(@TEMPLATE@_Table_t),
                              CFE_TBL_OPT_DEFAULT, ValidateExampleTbl);
    if (status != CFE_SUCCESS)
    {
        CFE_ES_WriteToSysLog("@TEMPLATE@: Error Registering Table, RC = 0x%08lX\n", (unsigned long)status);

        return status;
    }
    else
    {
        status = CFE_TBL_Load(@TEMPLATE@_Data.TblHandle, CFE_TBL_SRC_FILE, @TEMPLATE@_TBL_FILE);
    }
//EX2
...
//EX3,1,4,
    ...
    */
    CFE_TBL_Manage(@TEMPLATE@_Data.TblHandle);
        
    return CFE_SUCCESS;
    
} /* End @TEMPLATE@_ReportHousekeeping() */
//EX3
...
/*                                                                            */
/*  Purpose:                                                                  */
/*         Provide an example of accessing table data.                        */
/*                                                                            */
/* * * * * * * * * * * * * * * * * * * * * * * *  * * * * * * *  * *  * * * * */
static int32 SendTblDataEvent(void)
{
    ...
    int32               Status;
    @TEMPLATE@_Table_t *TblPtr;
    
    Status = CFE_TBL_GetAddress((void *)&TblPtr, @TEMPLATE@_Data.TblHandle);

    if (Status < CFE_SUCCESS)
    {
        CFE_ES_WriteToSysLog("@TEMPLATE@: Fail to get table address: 0x%08lx", (unsigned long)Status);
        return Status;
    }

    CFE_ES_WriteToSysLog("@TEMPLATE@: Table Value 1: %d  Value 2: %d", TblPtr->Int1, TblPtr->Int2);

    Status = CFE_TBL_ReleaseAddress(@TEMPLATE@_Data.TblHandle);
    if (Status != CFE_SUCCESS)
    {
        CFE_ES_WriteToSysLog("@TEMPLATE@: Fail to release table address: 0x%08lx", (unsigned long)Status);
        return Status;
    }
...
/* verifies the Int1 element is less than a maximum limit.         */
/*                                                                 */
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
int32 ValidateExampleTbl(void *TblData)
{
    int32               ReturnCode = CFE_SUCCESS;
    @TEMPLATE@_Table_t *TblDataPtr = (@TEMPLATE@_Table_t *)TblData;

    /*
    ** Example Table Validation
    */
    if (TblDataPtr->Int1 > @TEMPLATE@_TBL_INT1_MAX)
    {

        CFE_EVS_SendEvent(@TEMPLATE@_TABLE_VALIDATE_ERR_EID, CFE_EVS_EventType_ERROR,
                          "Table Int1 value %d exceeds validation limit %d",
                          TblDataPtr->Int1, @TEMPLATE@_TBL_INT1_MAX);
       
        /* First element is out of range, return an appropriate error code */
        ReturnCode = @TEMPLATE@_TBL_RANGE_ERR;
    }
