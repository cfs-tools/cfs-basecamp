...
static void ProcessExObjStack(void);
...
// Use CFE_EVS_NO_FILTER to remove a filter
static CFE_EVS_BinFilter_t  EventFilters[] =
{  
   /* Event ID                                Mask */
   {EXOBJ_EXECUTE_EID,                   CFE_EVS_FIRST_16_STOP}, 
   {EXOBJ_STACK_PUSH_EID,                CFE_EVS_FIRST_16_STOP},
   {@TEMPLATE@_PROCESS_EXOBJ_STACK_EID,  CFE_EVS_FIRST_16_STOP},
   ...
   */
   ...
   if (INITBL_Constructor(INITBL_OBJ, @TEMPLATE@_INI_FILENAME, &IniCfgEnum))
   {
      ...
      */
      ...
      OS_CountSemCreate(&@Template@.ChildExecSemaphore, "HELLO_CHILD_EXEC", 0, 0);
      EXOBJ_Constructor(EXOBJ_OBJ, INITBL_OBJ, @Template@.ChildExecSemaphore);
      
      CFE_ES_CreateChildTask(&@Template@.ChildTaskId, 
                             INITBL_GetStrConfig(INITBL_OBJ, CFG_CHILD_NAME),
                             EXOBJ_ChildTask, 0,
                             INITBL_GetIntConfig(INITBL_OBJ, CFG_CHILD_STACK_SIZE),
                             INITBL_GetIntConfig(INITBL_OBJ, CFG_CHILD_PRIORITY), 0);
      ...
      */
      ...
      CMDMGR_RegisterFunc(CMDMGR_OBJ, @TEMPLATE@_SET_CHILD_DELAY_CC,  EXOBJ_OBJ, EXOBJ_SetChildDelayCmd,  sizeof(@TEMPLATE@_SetChildDelay_CmdPayload_t));
      CMDMGR_RegisterFunc(CMDMGR_OBJ, @TEMPLATE@_SET_COUNTER_MODE_CC, EXOBJ_OBJ, EXOBJ_SetCounterModeCmd, sizeof(@TEMPLATE@_SetCounterMode_CmdPayload_t));
...
static void ProcessExObjStack(void)
{
   ...
   EXOBJ_CounterStackEntry_t CounterStackEntry;
   
   while(EXOBJ_StackPop(&CounterStackEntry))
   {
      CFE_EVS_SendEvent (@TEMPLATE@_PROCESS_EXOBJ_STACK_EID, CFE_EVS_EventType_INFORMATION,
                   "EXOBJ StackPop [%s]: %d",
                   CounterStackEntry.TimeStr, CounterStackEntry.CounterValue);    
   }

} /* End ProcessExObjStack() */
//EX2
